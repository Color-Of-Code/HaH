...
// ── LegacyNtpCheck ────────────────────────────────────────────────────────────
/// Detects the legacy ISC ntpd package. Modern systems should use chrony or
/// systemd-timesyncd, which handle VM clock skew and offer better security.
pub struct LegacyNtpCheck;

impl Check for LegacyNtpCheck {
    fn id(&self) -> &str {
        "legacy-ntp"
    }

    fn title(&self) -> &str {
        "Legacy NTP daemon (ntpd / ISC ntp)"
    }
    ...
    fn run(&self, ctx: &Context) -> CheckResult {
        ...
        if !is_package_installed(ctx.runner.as_ref(), "ntp") {
            return CheckResult::default();
        }

        let chrony_active = is_service_active(ctx.runner.as_ref(), "chrony");
        let timesyncd_active = is_service_active(ctx.runner.as_ref(), "systemd-timesyncd");
        let modern_active = chrony_active || timesyncd_active;

        let (description, severity) = if modern_active {
            let which = match (chrony_active, timesyncd_active) {
                (true, true) => "chrony and systemd-timesyncd",
                (true, false) => "chrony",
                _ => "systemd-timesyncd",
            };
            (
                format!(
                    "The legacy `ntp` (ISC ntpd) package is installed while {which} is also \
                     active. Multiple time-sync daemons compete to adjust the clock, which can \
                     cause instability. Remove the `ntp` package."
                ),
                Severity::Warning,
            )
        } else {
            (
                "The legacy `ntp` (ISC ntpd) package is installed. Modern alternatives such as \
                 `chrony` (recommended for servers and VMs) or `systemd-timesyncd` offer better \
                 accuracy, NTS/DNSSEC support, and resilience to large clock jumps."
                    .to_string(),
                Severity::Info,
            )
        };
        ...
        CheckResult::default().with_finding(Finding {
            id: "legacy-ntp".into(),
            title: "Legacy `ntp` (ISC ntpd) package is installed".into(),
            description,
            severity,
            remediation: Some(Remediation {
                description: "Remove ntp and enable chrony or systemd-timesyncd.".into(),
                commands: vec![
...
mod tests {
    ...
    #[test]
    fn legacy_ntp_check_id_and_title() {
        assert_eq!(LegacyNtpCheck.id(), "legacy-ntp");
        assert!(!LegacyNtpCheck.title().is_empty());
    }

    #[test]
    fn legacy_ntp_skips_non_debian() {
        assert!(LegacyNtpCheck.run(&non_debian_ctx()).findings.is_empty());
    }

    #[test]
    fn legacy_ntp_not_installed_returns_empty() {
        let mut runner = MockRunner::new();
        // dpkg-query returns "not installed"
        runner
            .expect_run()
    ...
    #[test]
    fn legacy_ntp_installed_no_modern_service() {
        let mut runner = MockRunner::new();
        ...
        runner.expect_run().returning(move |_, _| {
            ...
            if n == 0 {
                Ok(ok_output("install ok installed")) // ntp installed
            } else {
                Ok(failure_output()) // neither chrony nor timesyncd active
            }
        });
        let result = LegacyNtpCheck.run(&debian_ctx(Arc::new(runner)));
        assert_eq!(result.findings.len(), 1);
        assert_eq!(result.findings[0].severity, Severity::Info);
    }

    #[test]
    fn legacy_ntp_installed_with_chrony_active() {
        let mut runner = MockRunner::new();
        let call = std::sync::atomic::AtomicUsize::new(0);
        runner.expect_run().returning(move |_, _| {
            let n = call.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            if n == 0 {
                ...
                Ok(success_output()) // chrony active
            } else {
                Ok(failure_output()) // timesyncd inactive
            }
        });
        let result = LegacyNtpCheck.run(&debian_ctx(Arc::new(runner)));
        assert_eq!(result.findings.len(), 1);
    ...
    #[test]
    fn legacy_ntp_installed_with_timesyncd_active() {
        let mut runner = MockRunner::new();
        let call = std::sync::atomic::AtomicUsize::new(0);
        runner.expect_run().returning(move |_, _| {
            let n = call.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            if n == 0 {
                ...
                Ok(failure_output()) // chrony inactive
            } else {
                Ok(success_output()) // timesyncd active
            }
        });
        let result = LegacyNtpCheck.run(&debian_ctx(Arc::new(runner)));
        assert_eq!(result.findings.len(), 1);
    ...
    #[test]
    fn legacy_ntp_installed_with_both_modern_services() {
        let mut runner = MockRunner::new();
        let call = std::sync::atomic::AtomicUsize::new(0);
        runner.expect_run().returning(move |_, _| {
            let n = call.fetch_add(1, std::sync::atomic::Ordering::SeqCst);
            if n == 0 {
                Ok(ok_output("install ok installed"))
            }
            // ntp
            else {
                Ok(success_output())
            } // both chrony + timesyncd active
        });
        let result = LegacyNtpCheck.run(&debian_ctx(Arc::new(runner)));
        assert_eq!(result.findings.len(), 1);
        assert_eq!(result.findings[0].severity, Severity::Warning);
